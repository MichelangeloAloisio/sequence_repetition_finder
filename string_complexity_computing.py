...
def single_repetitions(range_, start_coordinate, string,dict_single_repetitions):
	list_repetitions=string[start_coordinate:start_coordinate+range_]
	for x in Counter(list_repetitions).values():
		if x==range_:
...
def list_positions(x,y,number_repetitions_to_search, list_count):
	...
	if len(lista_repetitions)==number_repetitions_to_search*2:
		...
		if len(splitted_text_in_subtext_CTRL_or_TWO)==2:
			if len(splitted_text_in_subtext_CTRL_or_TWO[0])==number_repetitions_to_search and len(splitted_text_in_subtext_CTRL_or_TWO[0])==number_repetitions_to_search:
...
for x in file:
	if len(x)<=1:
	    ...
	    if '>' in x:
	    	...
	    	print('SEQUENCE_ANALIZING: ',x.strip())
	    else:
	    	formatted_x=x.lower()
	    	count=0
